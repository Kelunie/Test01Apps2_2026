...
export default function Carrito({ navigation }) {
	...
	const categories = useMemo(() => {
		const unique = Array.from(new Set(cartItems.map((item) => item?.category).filter(Boolean)));
		return unique;
	}, [cartItems]);
		...
		<View style={stylesCart.container}>
			<View style={stylesCart.headerRow}>
				<View style={stylesCart.headerIconWrap}>
					<MaterialCommunityIcons name="cart-outline" style={stylesCart.headerIcon} color="#2f4f92" />
				</View>
				<View>
			...
			{isLoading ? (
				...
				<FlatList
					...
					renderItem={({ item }) => (
						<TouchableOpacity
							onPress={() => navigation.navigate("DetailProd", { product: item })}
							style={stylesCart.card}
						>
							<Image source={{ uri: item.image }} resizeMode="contain" style={stylesCart.cardImage} />
							<Text style={stylesCart.codeText}>Código: #{item.id}</Text>
							<Text numberOfLines={2} style={stylesCart.cardTitle}>{item.title}</Text>
							<Text style={stylesCart.cardInfo}>Cant: {item.quantity || 1}</Text>
							<Text style={stylesCart.cardInfo}>Categoría: {item.category || "-"}</Text>
					...
					ListEmptyComponent={<Text style={stylesCart.emptyText}>No hay productos en el carrito.</Text>}
					ListFooterComponent={
						cartItems.length > 0 ? (
							<View style={stylesCart.summaryCard}>
								<Text style={stylesCart.summaryTitle}>Resumen de compra</Text>
								<Text style={stylesCart.summaryText}>Productos: {cartItems.length}</Text>
								<Text style={stylesCart.summaryText}>Código RNG: {purchaseCode || generatePurchaseCode()}</Text>
								<Text style={stylesCart.summaryTotal}>Total: ${totalAmount.toFixed(2)}</Text>
								<TouchableOpacity style={stylesCart.confirmButton} onPress={onPressConfirmPurchase}>
									<Text style={stylesCart.confirmButtonText}>Confirmar compra</Text>
								</TouchableOpacity>
							</View>
						) : null
					}
