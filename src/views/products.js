...
export default function Products({ navigation }) {
	...
	const loadProducts = async (refreshing = false) => {
		...
		try {
			...
			const { data: categoryData } = await axios.get(ENDPOINTS.products.categories);
		...
		<View style={stylesProducts.container}>
			<View style={stylesProducts.headerRow}>
				<View style={stylesProducts.logoCircle}>
					<MaterialCommunityIcons name="storefront-outline" style={stylesProducts.logoIcon} color="#6a247f" />
				</View>
				<View>
					<Text style={stylesProducts.title}>Fake Store</Text>
					<Text style={stylesProducts.name}>{displayName ? `Nombre: ${displayName}` : "Sesión activa"}</Text>
				</View>
			</View>
				...
				<FlatList
					...
					renderItem={({ item }) => (
						<View style={stylesProducts.productCard}>
							<TouchableOpacity onPress={() => navigation.navigate("DetailProd", { product: item })}>
								<Image
					...
					ListEmptyComponent={<Text style={stylesProducts.emptyText}>No hay productos para mostrar.</Text>}
				/>
			)}
		</View>
	);
